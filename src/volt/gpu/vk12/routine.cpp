...
routine<T, Sub>::routine(std::shared_ptr<gpu::pool<T>> &&pool)
		: gpu::routine<T, Sub>(std::move(pool)) {
...
// 	VOLT_GPU_DEBUG_ASSERT(vkBeginCommandBuffer(command_buffer, &begin_info)
...
template<command_type T, bool Sub>
void routine<T, Sub>::execute_subroutine(const std::shared_ptr<gpu::subroutine<T>> &subroutine) {
	if constexpr (!Sub) {
