...
pool<T>::pool(std::shared_ptr<video::queue<T>> &&queue)
		: video::pool<T>(std::move(queue)) {
...
template<command_type T>
std::shared_ptr<video::routine<T>> pool<T>::create_routine() {
	return std::shared_ptr<video::routine<T>>(
			new typename vk12::routine<T>(pool<T>::shared_from_this()));
}

template<command_type T>
std::shared_ptr<video::subroutine<T>> pool<T>::create_subroutine() {
	return std::shared_ptr<video::subroutine<T>>(
